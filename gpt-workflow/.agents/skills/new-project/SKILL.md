...
根据当前请求中的目标、材料和参数执行。默认建立研究设定并完成第一阶段具体工作；执行范围由用户任务决定，不从一个想法自动跑到投稿。
...
需要全新目录时，在本版工作流库运行 `python3 scripts/new_project.py <destination> --lang stata`，语言可选 python、r、julia；已有目录不覆盖。真实研究项目内无需再次初始化，直接更新 brief 并开展已授权工作。入口是 AGENTS.md、.agents/skills/ 和 .codex/agents/，所有参考在本项目 references/。模型、权限和 MCP 保留用户配置；没有子 agent 能力时明示串行。
