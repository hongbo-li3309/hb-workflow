...
Use `references/coding-standards-stata.md` for a main/subgroup example that executes `estimates restore m_baseline` before `estimates save`. `restore` only restores data. Stata `tempfile` does not survive as a handoff into the next session.

Use `references/coding-standards-python.md` for a runnable descriptive example and explicit `rng = np.random.default_rng(SEED)`; draw through `rng`. Discarding the object does not seed later global draws.
