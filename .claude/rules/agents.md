...
Critic 不改主稿、数据、代码或用户决定；默认返回关键发现，只有分配了明确留存任务才写自己负责的报告。普通任务由主线程完成执行与质疑，不为二者的角色区分强行启动两个 agent。权限与报告中声称的工作必须一致，只读检查不能称已运行。Worker 自检仍必要，重要方法或结果可请独立 critic 核验；小修直接完成并核对。
