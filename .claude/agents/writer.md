...
重大稿件可由主会话调 `writer-critic` 独立审查；你不自评通关。普通审查由主会话在对话中整合；用户要求、正式评审或重要交接确需留存时，再合并存入一份 `quality_reports/reviews/YYYY-MM-DD_<task>.md`，不逐次调用生成文件。挑少量重要改句解释表达与经济含义的关系，帮助 Hongbo 学会判断，不为每一句附教程。
