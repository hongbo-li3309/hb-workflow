...
重要新主张、设计、代码或证明根据风险交对应 critic 审查；普通低影响编辑不机械启动完整配对。critic 独立检查并返回发现，主会话按留存价值决定是否保存，worker 完成修复；工具核验与方法判断分别记录。子 agent 自称完成还需核对实际产物和证据。

## 路由

下表是可选分工，不是必经流程。日常由主线程先完成一个可判断的小步骤，同时提出最重要的疑问；独立审查确有价值时才委派 critic。产物可以直接是对话结果，不要求创建文件或独立报告。
