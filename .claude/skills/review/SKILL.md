...
argument-hint: "[target] [--peer journal | --stress journal | --r2 | --methods | --theory | --proofread | --code | --replicate language | --all]"
allowed-tools: Read, Grep, Glob, Write, Bash, Agent, WebSearch, WebFetch
---
# Review

Read the target/version and current `research/PROJECT_BRIEF.md` when present. Apply `.claude/rules/quality.md`; findings distinguish demonstrated errors, missing evidence and editorial suggestions. Save a report to `quality_reports/reviews/YYYY-MM-DD_<task>.md` only when useful. Do not edit the manuscript during review.
